...
const route = "/landlord";

const createLandlord = async (body) => {
    const requestBody = createLandlordBody(body);

    return await apiRequest.post(route, requestBody);
}

const updateLandlord = async (id, body) => {
    const requestBody = createLandlordBody(body);

    return await apiRequest.put(route + `/${id}`, requestBody);
}

const deleteLandlord = async (id, body) => {
    return await apiRequest.delete(route + `/${id}`, body);
}

const getLandlords = async (id = null) => {
    return await apiRequest.get(route + (id ? `/${id}` : ""));
}

const singlePageLoader = async ({request, params}) => {
    return (await getLandlords(params.id)).data;
}

const listPageLoader = ({ request, params }) => {
  const query = request.url.split("?")[1];
  const landlordPromise = apiRequest.get(route + "?" + query);

  return defer({
    landlordResponse: landlordPromise
  });
}
...
function createLandlordBody(data) {
